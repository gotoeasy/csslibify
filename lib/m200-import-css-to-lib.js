	...
	return function imp(fileOrCss, opt={}){
        ...
        if ( cacheValue ) {
            if ( !this._imported.includes(cacheKey) ) {   // 避免重复导入
                ...
                if ( oJson.fontfaces ) {
                    (oJson.fontfaces || []).forEach(node => node.toString = bus.at('template-to-tostring', node.template));   // 添加toString方法
                    this.fontfaces = merge(this.fontfaces, oJson.fontfaces);
                }
                if ( oJson.keyframes ) {
                ...
                if ( oJson.atviewports ) {
                    (oJson.atviewports || []).forEach(node => node.toString = bus.at('template-to-tostring', node.template ));   // 添加toString方法
                    this.atviewports = merge(this.atviewports, oJson.atviewports);
                }
            }
        ...
        // 取出处理结果
        processResult.fontfaces && (this.fontfaces = merge(this.fontfaces, processResult.fontfaces));
        ...
        processResult.atviewports && (this.atviewports = merge(this.atviewports, processResult.atviewports));

        if ( rs.css ) {
            let filetodo = `${cache.path}/todo/${hash(rs.css)}.css`;
            if ( !File.existsFile(filetodo) ) {
