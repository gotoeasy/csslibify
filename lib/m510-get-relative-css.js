    ...
    return function (...args) {
        ...
        args.forEach(arg => {
            if ( typeof arg === 'string' ) {
                ...
                atpage = !!arg.atpage;
                atviewport = !!arg.atviewport;
                arg.strict !== undefined && (strict = !!arg.strict);    // 指定匹配模式时按指定的来（默认是严格匹配）
            }else{
                // 忽略无效参数
            }
        });
        ...
        // 按需查找引用
        if ( !requireClasses.length && !requireTags.length && !atpage && !atviewport ) {
        ...
        atviewport && rs.push(getAtviewport()); // 设定文档大小用的@viewport样式
